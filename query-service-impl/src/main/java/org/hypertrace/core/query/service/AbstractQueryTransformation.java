...
public abstract class AbstractQueryTransformation implements QueryTransformation {
  ...
  protected Single<Filter> transformFilter(Filter filter) {
    ...
    if(lhsExpression.getValueCase() == Expression.ValueCase.ATTRIBUTE_EXPRESSION) {
      ...
      if (lhsAttributeId.substring(lhsAttributeId.lastIndexOf(".") + 1).equals("startTime")) {
        lhsExpression = Expression.newBuilder().setAttributeExpression(AttributeExpression.newBuilder().setAttributeId(lhsAttributeId.concat("Filter")).build()).build();
      }
    }
